...
namespace Senparc.Xncf.CodeBuilder.Services
{
    public class DbExtension
    {
        ...
        public DbExtension(DbContext context)
        {
            _context = context;
        }
        ...
        private IList<SysTableColumn> GetMySqlStructure(string tableName)
        {
            ...
            //return (IList<SysTableColumn>)columns.ToList();
        ...
        public static IList<T> SqlQuery<T>(DbContext db, string sql)
            where T : new()
        {
        ...
        private IList<SysTableColumn> GetSqlServerStructure(string tableName)
        {
            ...
            SELECT TableName,
                ...
                CASE WHEN ColumnType = 'uniqueidentifier' THEN 'guid'
                     ...
                     WHEN ColumnType IN('time', 'date', 'DATETIME', 'smallDATETIME','datetime2')
                     THEN 'DateTime'
                ...
                END as  EntityType,
                    ...
                    [Maxlength],
                IsKey,
                CASE WHEN ColumnName IN('CreateID', 'ModifyID', 'AddUserId', 'UpdateUserId', 'Flag',  '')
                          OR IsKey = 1 THEN 0
                     ELSE 1
                END AS IsDisplay ,
				1 AS IsColumnData,
        ...
        FROM    (SELECT obj.name AS TableName ,
            ...
            var columns = SqlQuery<SysTableColumn>(_context, sql);
            //var columns = _context.Database.Query<SysTableColumn>().FromSqlRaw(sql);
            return (IList<SysTableColumn>)columns.ToList();
            //return new List<SysTableColumn>();
        }
    }
