...
Count : 0,
    ...
    methods: {
        ...
        getList() {
            ...
            service.get("/Admin/UserBuilderLog/Index?handler=List", { params: data}).then(res => {
                if (res.data.success) {
        ...
        handleEdit(index, row) {
            ...
            if (row) {
                // 编辑
                let { Id,Flag,AddTime,LastUpdateTime,AdminRemark,Remark,AdditionNote,TenantId,UserId,TableName,ModuleName,Path,Count, } = row;
                this.dialog.data = {
                    Id,Flag,AddTime,LastUpdateTime,AdminRemark,Remark,AdditionNote,TenantId,UserId,TableName,ModuleName,Path,Count,
                };
                this.dialog.title = '编辑生成记录';
                this.dialog = Object.assign({}, this.dialog);
            } else {
                // 新增
                this.dialog.title = '新增生成记录';
            }
        },
        // 更新新增编辑
        updateData() {
...
AdditionNote : this.dialog.data.AdditionNote,
TenantId : this.dialog.data.TenantId*1,
UserId : this.dialog.data.UserId,
TableName : this.dialog.data.TableName,
ModuleName : this.dialog.data.ModuleName,
Path : this.dialog.data.Path,
Count : this.dialog.data.Count*1,

                    };
                    service.post("/Admin/UserBuilderLog/Index?handler=Save", data).then(res => {
                        if (res.data.success) {
                            this.getList();
                            this.$notify({
                                title: "Success",
        ...
        handleDelete(index, row) {
            let ids = [row.id];
            service.post("/Admin/UserBuilderLog/Index?handler=Delete", ids).then(res => {
                if (res.data.success) {
                    this.getList();
                    this.$notify({
                        title: "Success",
