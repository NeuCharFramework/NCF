...
namespace Senparc.Xncf.User.Areas.User.Pages
{
    public class XncfModuleIndexModel : BaseAdminPageModel
    {
        ...
        /// 数据库已存的XncfModules
        /// </summary>
        public PagedList<Senparc.Ncf.Core.Models.DataBaseModel.XncfModule> XncfModules { get; set; }
        public List<IXncfRegister> NewXncfRegisters { get; set; }

        private void LoadNewXncfRegisters(PagedList<Senparc.Ncf.Core.Models.DataBaseModel.XncfModule> xncfModules)
        {
        ...
        public async Task<IActionResult> OnGetMofulesAsync(int pageIndex = 0, int pageSize = 0)
        {
            //更新菜单缓存
            await _sysMenuService.GetMenuDtoByCacheAsync(true).ConfigureAwait(false);
            PagedList<Senparc.Ncf.Core.Models.DataBaseModel.XncfModule> xncfModules = await _xncfModuleServiceEx.GetObjectListAsync(pageIndex, pageSize, _ => true, _ => _.AddTime, Ncf.Core.Enums.OrderingType.Descending);
