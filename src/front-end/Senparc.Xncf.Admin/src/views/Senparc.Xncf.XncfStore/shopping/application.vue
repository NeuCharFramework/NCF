<template>
  <div>
    <!-- 个人应用-全部 -->
    <div class="myapplicationall" v-if="!showmian">
      <!-- 标题 -->
      <el-card>
    ...
    <!-- 个人应用-详情页 -->
    <div class="myapplication" v-if="showmian">
      <!-- 标题 -->
      <el-card>
        <h3 class="shoptit">应用形式-微信API-SWAG文档</h3>
      </el-card>
...
export default {
  ...
  data() {
    return {
      showmian: false, //展示首页false，详情页true
      // 详情页数据
      tableData: [
        {
  ...
  created() {
    this.seeStatus(); //查看路由配置
  },
  methods: {
    seeStatus() {
      console.log(this.$router.currentRoute.query.datail);
      if (this.$router.currentRoute.query.datail) {
        this.showmian = this.$router.currentRoute.query.datail;
      }
    },
    submitForm(formName) {
...
/* 首页的 */
.myapplicationall >>> .el-card__body {
  padding: 0px;
  /* margin-bottom: 10px; */
}
</style>