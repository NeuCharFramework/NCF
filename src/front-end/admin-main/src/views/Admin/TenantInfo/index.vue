    ...
    <div class="admin-tenant-info">
      <div>
        ...
        <el-row v-show="!enableMultiTenant" :gutter="20">
          <el-col :span="6">
            <div class="grid-content bg-purple">当前系统已关闭多租户</div>
          </el-col>
        </el-row>
        <el-row v-show="enableMultiTenant" :gutter="20">
          <el-col :span="6">
            <div class="grid-content bg-purple">当前系统匹配规则：<span>{{ tenantRule }}</span></div>
          </el-col>
          <el-col :span="6">
            <div class="grid-content bg-purple">名称：<span>{{ requestTenantInfo.name }} <span class="float-right">(ID:{{ requestTenantInfo.id }})</span></span></div>
          </el-col>
          <el-col :span="6">
            <div class="grid-content bg-purple">匹配关键字：<span>{{ requestTenantInfo.tenantKey }}</span></div>
          </el-col>
          <el-col :span="6">
            <div class="grid-content bg-purple">匹配时间：<span>{{ requestTenantInfo.beginTime }}</span></div>
          </el-col>
        </el-row>
      </div>
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column align="center" label="管理员备注">
          <template slot-scope="scope">
            {{ scope.row.adminRemark }}
          </template>
        </el-table-column>
        <el-table-column align="center" label="启用" width="60">
          <template slot-scope="scope">
            {{ scope.row.enable }}
          </template>
        </el-table-column>
