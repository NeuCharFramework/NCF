<template>
  ...
  <div class="extend">
            ...
            <ul>
              ...
              <li>
                ...
                <template v-if="
                  ...
                  ">
                  <el-button size="mini" :type="data.xncfModule.state === 0 ? 'success' : 'warning'" @click="updataState('1')">
    ...
    <!-- 执行弹窗 -->
    <el-dialog width="60%" v-if="run.data.key" :title="run.data.key.name" :visible.sync="run.visible" class="run-dialog" @close="runVisibleClose" :close-on-click-modal="false">
      <h4><i class="fa fa-newspaper-o"></i>{{ run.data.key.description }}</h4>
      <el-form :label-position="'left'">
        <el-form-item v-for="item in run.data.value" :key="item.name" :label="item.title || item.name" label-width="130px" :required="item.isRequired">
