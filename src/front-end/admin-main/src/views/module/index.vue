      ...
      <el-main>
        <el-table :data="oldTableData" v-if="oldTableData.length>0" style="width: 100%;margin-bottom: 20px;">
          <el-table-column align="left" label="菜单名称/版本号">
            <template slot-scope="scope">
              <div>
                <i :class="scope.row.icon?scope.row.icon:'fa fa-chain-broken'"></i>
                {{scope.row.menuName}}
              </div>
              <div>
                <i class="fa fa-code-fork"></i>
...
<script>
import { getModuleList, getUnModuleList } from '@/api/module'
export default {
  data() {
    return {
      newTableData: [], // 新模块数据
  ...
  methods: {
    ...
    async handleInstall(index, row) {
      await service.get(
        `/Admin/XncfModule/Index?handler=ScanAjax&uid=${row.uid}`
      )
    ...
    handleHandle(index, row) {
      window.location.href =
        '/Admin/XncfModule/Start/?uid=' + row.xncfRegister.uid
    },
    // 主页
