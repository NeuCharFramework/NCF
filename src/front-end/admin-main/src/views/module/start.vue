        ...
        <el-aside class="module-aside">
          <div class="start-des">
            <ul>
              ...
              <li v-if="data.xncfModule.version!==data.xncfRegister.version">发现新版本:v{{data.xncfRegister.version}} <el-button size="mini" @click="updataVersion"> <em class="fa fa-gift"></em>立即更新</el-button>
              </li>
              <li>
              ...
              <li>
                ...
                <el-tooltip v-for="item in data.xncfRegister.interfaces" :key="item" class="item" effect="dark" placement="top">
                  <div slot="content">
                    {{tooltip[item]}}
              ...
              <li>
                <i class="fa fa-map-marker fa-sliders" ></i><span :class="[data.xncfModule.state===1?'start-green':'start-gray']" style="margin-right:10px;">{{xNcfModules_State[data.xncfModule.state]}}</span>
                ...
                <template v-if="data.xncfModule.state===1">
                  <el-button size="mini"  type="danger" @click="updataState('0')"> <em class="glyphicon glyphicon-off"></em> 关闭</el-button>
                </template>
              </li>
              <li>
          ...
          <div class="start-log">
            <el-collapse>
              <el-collapse-item title="更新记录" name="1">
                <el-table :data="data.xncfModuleUpdateLog" stripe style="width: 100%">
                  ...
                  <div class="item-container">
                    <el-collapse>
                      <el-collapse-item title="页面透视" name="2">
                        <el-menu default-active="2" class="el-menu-vertical-demo" background-color="#304156" text-color="#bfcbd9" width="300px" active-text-color="#409eff">
                          <el-menu-item v-for="(item,index) in data.xncfRegister.areaPageMenuItems" :key="index">
                            <a :href="item.url">
        ...
        <el-form-item v-for="item in run.data.value" :key="item.name" :label="item.name" label-width="130px" :required="item.isRequired">
          ...
          <el-select v-if="item.parameterType===1" v-model="runData[item.name].value">
            <template v-if="item.selectionList.items">
              <el-option v-for="selectionItem in item.selectionList.items" :key="selectionItem.text" :label="selectionItem.text" :value="selectionItem.value"></el-option>
            </template>
          </el-select>
...
export default {
  ...
  methods: {
    async getList() {
      ...
      // 获取详情
      const res = await getItemModule(uid)
