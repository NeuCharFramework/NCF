...
{
    public class CourseChapterService : ServiceBase<CourseChapter>
    {
        public CourseChapterService(IRepositoryBase<CourseChapter> repo, IServiceProvider serviceProvider) : base(repo, serviceProvider)
        {
        }
        //TODO: 更多业务方法可以写到这里
        public async Task CreateOrUpdateAsync(AddOrUpdateCourseChapterReq req)
        {
            CourseChapter entity;
            if (req.Id == 0)
            {
                entity = new CourseChapter(req);
