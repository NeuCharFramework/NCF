...
{
    public class VideoUserCommentService : ServiceBase<VideoUserComment>
    {
        public VideoUserCommentService(IRepositoryBase<VideoUserComment> repo, IServiceProvider serviceProvider) : base(repo, serviceProvider)
        {
        }
        //TODO: 更多业务方法可以写到这里
        public async Task CreateOrUpdateAsync(AddOrUpdateVideoUserCommentReq req)
        {
            VideoUserComment entity;
            if (req.Id == 0)
            {
                entity = new VideoUserComment(req);
       ...
       entity.RenderContent= req.RenderContent;
