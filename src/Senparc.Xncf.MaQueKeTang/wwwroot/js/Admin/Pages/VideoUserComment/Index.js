...
originalContent:[{required:true, message:"原始内容为必填项",trigger:"blur"}],
renderContent:[{required:true, message:"渲染后的内容为必填项",trigger:"blur"}],
...
tenantId:[{required:true, message:"租户ID为必填项",trigger:"blur"}],
    ...
    methods: {
        ...
        getList() {
            ...
            service.get("/Admin/VideoUserComment/Index?handler=List", { params: data}).then(res => {
                if (res.data.success) {
        ...
        handleEdit(index, row) {
            ...
            if (row) {
                // 编辑
                let { id,userId,videoId,originalContent,renderContent,flag,addTime,addUserId,addUserName,lastUpdateTime,updateUserId,updateUserName,adminRemark,remark,tenantId,deleted_at, } = row;
                this.dialog.data = {
                    id,userId,videoId,originalContent,renderContent,flag,addTime,addUserId,addUserName,lastUpdateTime,updateUserId,updateUserName,adminRemark,remark,tenantId,deleted_at,
                };
                this.dialog.title = '编辑视频评论';
                this.dialog = Object.assign({}, this.dialog);
            } else {
                // 新增
                this.dialog.title = '新增视频评论';
            }
        },
        // 更新新增编辑
        updateData() {
...
originalContent : this.dialog.data.originalContent,
renderContent : this.dialog.data.renderContent,
...
deleted_at : this.dialog.data.deleted_at,
                    ...
                    service.post("/Admin/VideoUserComment/Index?handler=Save", data).then(res => {
                        if (res.data.success) {
                            this.getList();
                            this.$notify({
                                title: "Success",
        ...
        handleDelete(index, row) {
            let ids = [row.id];
            service.post("/Admin/VideoUserComment/Index?handler=Delete", ids).then(res => {
                if (res.data.success) {
                    this.getList();
                    this.$notify({
                        title: "Success",
