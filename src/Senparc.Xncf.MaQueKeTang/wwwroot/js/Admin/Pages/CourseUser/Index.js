...
Id : 0,
    ...
    methods: {
        ...
        getList() {
            ...
            service.get("/Admin/CourseUser/Index?handler=List", { params: data}).then(res => {
                if (res.data.success) {
        ...
        handleEdit(index, row) {
            ...
            if (row) {
                // 编辑
                let { AccountId,CourseId,charge,Flag,AddTime,LastUpdateTime,AdminRemark,Remark,TenantId,Id, } = row;
                this.dialog.data = {
                    AccountId,CourseId,charge,Flag,AddTime,LastUpdateTime,AdminRemark,Remark,TenantId,Id,
                };
                this.dialog.title = '编辑课程用户';
                this.dialog = Object.assign({}, this.dialog);
            } else {
                // 新增
                this.dialog.title = '新增课程用户';
            }
        },
        // 更新新增编辑
        updateData() {
...
Id : this.dialog.data.Id*1,
                    ...
                    service.post("/Admin/CourseUser/Index?handler=Save", data).then(res => {
                        if (res.data.success) {
                            this.getList();
                            this.$notify({
                                title: "Success",
        ...
        handleDelete(index, row) {
            let ids = [row.id];
            service.post("/Admin/CourseUser/Index?handler=Delete", ids).then(res => {
                if (res.data.success) {
                    this.getList();
                    this.$notify({
                        title: "Success",
