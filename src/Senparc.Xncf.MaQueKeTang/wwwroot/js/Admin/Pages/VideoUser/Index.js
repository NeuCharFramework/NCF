...
charge:[{required:true, message:"收费为必填项",trigger:"blur"}],
...
tenantId:[{required:true, message:"租户Id为必填项",trigger:"blur"}],
    ...
    methods: {
        ...
        getList() {
            ...
            service.get("/Admin/VideoUser/Index?handler=List", { params: data}).then(res => {
                if (res.data.success) {
        ...
        handleEdit(index, row) {
            ...
            if (row) {
                // 编辑
                let { id,userId,videoId,charge,flag,addTime,addUserId,addUserName,lastUpdateTime,updateUserId,updateUserName,adminRemark,remark,tenantId,deleted_at, } = row;
                this.dialog.data = {
                    id,userId,videoId,charge,flag,addTime,addUserId,addUserName,lastUpdateTime,updateUserId,updateUserName,adminRemark,remark,tenantId,deleted_at,
                };
                this.dialog.title = '编辑视频用户';
                this.dialog = Object.assign({}, this.dialog);
            } else {
                // 新增
                this.dialog.title = '新增视频用户';
            }
        },
        // 更新新增编辑
        updateData() {
...
deleted_at : this.dialog.data.deleted_at,
                    ...
                    service.post("/Admin/VideoUser/Index?handler=Save", data).then(res => {
                        if (res.data.success) {
                            this.getList();
                            this.$notify({
                                title: "Success",
        ...
        handleDelete(index, row) {
            let ids = [row.id];
            service.post("/Admin/VideoUser/Index?handler=Delete", ids).then(res => {
                if (res.data.success) {
                    this.getList();
                    this.$notify({
                        title: "Success",
