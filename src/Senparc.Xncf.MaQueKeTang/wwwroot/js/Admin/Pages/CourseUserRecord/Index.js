...
Id : 0,
    ...
    methods: {
        ...
        getList() {
            ...
            service.get("/Admin/CourseUserRecord/Index?handler=List", { params: data}).then(res => {
                if (res.data.success) {
        ...
        handleEdit(index, row) {
            ...
            if (row) {
                // 编辑
                let { CourseId,AccountId,deleted_at,is_watched,watched_at,progress,Flag,AddTime,LastUpdateTime,AdminRemark,Remark,TenantId,Id, } = row;
                this.dialog.data = {
                    CourseId,AccountId,deleted_at,is_watched,watched_at,progress,Flag,AddTime,LastUpdateTime,AdminRemark,Remark,TenantId,Id,
                };
                this.dialog.title = '编辑课程用户记录';
                this.dialog = Object.assign({}, this.dialog);
            } else {
                // 新增
                this.dialog.title = '新增课程用户记录';
            }
        },
        // 更新新增编辑
        updateData() {
...
is_watched : this.dialog.data.is_watched,
watched_at : this.dialog.data.watched_at,
progress : this.dialog.data.progress,
...
Id : this.dialog.data.Id*1,
                    ...
                    service.post("/Admin/CourseUserRecord/Index?handler=Save", data).then(res => {
                        if (res.data.success) {
                            this.getList();
                            this.$notify({
                                title: "Success",
        ...
        handleDelete(index, row) {
            let ids = [row.id];
            service.post("/Admin/CourseUserRecord/Index?handler=Delete", ids).then(res => {
                if (res.data.success) {
                    this.getList();
                    this.$notify({
                        title: "Success",
