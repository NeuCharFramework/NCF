...
namespace Senparc.Areas.Admin.Areas.Admin.Pages
{
    ...
    /// 课程用户收藏
    ///</summary>
    public class CourseUserLike_IndexModel : Senparc.Ncf.AreaBase.Admin.AdminXncfModulePageModelBase
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly CourseUserLikeService _courseUserLikeService;
        public CourseUserLike_IndexModel(IServiceProvider serviceProvider, CourseUserLikeService courseUserLikeService, Lazy<XncfModuleService>
         xncfModuleService) : base(xncfModuleService)
        {
            _serviceProvider = serviceProvider;
            _courseUserLikeService = courseUserLikeService;
        }
        /// <summary>
        /// 列表
        /// </summary>
        ...
        /// <returns></returns>
        [Ncf.AreaBase.Admin.Filters.CustomerResource("courseuserlike-search")]
        //public async Task<IActionResult> OnGetListAsync(int pageIndex, int pageSize)
        public async Task<IActionResult> OnGetListAsync([FromQuery] QueryCourseUserLikeListReq req)
        {
            var seh = new SenparcExpressionHelper<CourseUserLike>();
            seh.ValueCompare.AndAlso(!string.IsNullOrEmpty(req.key), _ => _.Remark.Contains(req.key));
            var where = seh.BuildWhereExpression();
            var ObjectList = await _courseUserLikeService.GetObjectListAsync(req.pageIndex, req.pageSize, where, req.orderField);
...
properties.Add(new KeyDescription { Key = "Id", Description = "Id", Browsable = true, Type = "int"});
        ...
        /// </summary>
        /// <returns></returns>
        [Ncf.AreaBase.Admin.Filters.CustomerResource("courseuserlike-add", "courseuserlike-edit")]
        public async Task<IActionResult> OnPostSaveAsync([FromBody] AddOrUpdateCourseUserLikeReq req)
        {
            await _courseUserLikeService.CreateOrUpdateAsync(req);
            return Ok(true);
        }
        /// <summary>
        /// 删除
        ...
        /// <returns></returns>
        [Ncf.AreaBase.Admin.Filters.CustomerResource("courseuserlike-delete")]
        public IActionResult OnPostDelete([FromBody] int[] ids)
        {
            foreach (var id in ids)
            {
                _courseUserLikeService.DeleteObject(id);
            }
